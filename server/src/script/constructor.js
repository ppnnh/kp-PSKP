...
function insertIngredient(e) {
	...
	tag.id = `${ingredient}`;
	tag.className = 'constructor-image';
	tag.width = '300';
	let sauce = document.getElementById(`Sauce`);
	let cheese = document.getElementById(`Cheese`);
	if (
		document.getElementById(`${ingredient}`) == undefined &&
		sauce == undefined &&
		cheese == undefined
	) {
		base.after(tag);
		totalPrice += parseFloat(
			document.getElementById(`price ${ingredient}`).textContent
		);
		totalWeight += parseFloat(
			document.getElementById(`weight ${ingredient}`).textContent
		);
